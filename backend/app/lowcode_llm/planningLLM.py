    ...
    def _txt2json(self, workflow_txt):
        ...
        try:
            ...
            for step in steps:
                if step[0:4] != "STEP":
                    continue
                left_indices = [_.start() for _ in re.finditer(r"\[", step)]
                right_indices = [_.start() for _ in re.finditer(r"\]", step)]
                ...
                if re.findall(re.compile(r'[A-Za-z]', re.S), jump_str) == []:
                    workflow.append({"stepId": step_id,
                                     "stepName": step_name,
                    ...
                    continue
                jump_logic = []
                left_indices = [_.start() for _ in re.finditer(r'\[', jump_str)]
                right_indices = [_.start() for _ in re.finditer(r'\]', jump_str)]
                i = 1
                while i < len(left_indices):
                    jump = {"Condition": jump_str[left_indices[i]+1: right_indices[i-1]],
                            "Target": re.search(r'STEP\s\d', jump_str[left_indices[i+1]+1: right_indices[i]]).group(0)}
                    jump_logic.append(jump)
                    i += 3
                workflow.append({"stepId": step_id,
                                 "stepName": step_name,
                                 "stepDescription": step_description,
                                 "jumpLogic": jump_logic,
                                 "extension": []})
